...
def load_and_merge_lora(base_model_path: str, lora_path: str) -> Dict[str, torch.Tensor]:
    ...
    if device == "cpu":
        logger.warning("CUDA not detected, running on CPU. Performance will be slow.")
    try:
    ...
    for submodule_name, tensors in submodule_tensors.items():
        if 'A' in tensors and 'B' in tensors:
            ...
            logger.warning(f"Submodule '{submodule_name}' missing matrix A or B, skipped.")
...
def calculate_lora_delta_metrics(
    ...
    if metric not in supported_metrics:
        raise ValueError(f"Unsupported metric: '{metric}'. Supported values are: {supported_metrics}")
    results = {}
...
def main(
    ...
    # The Python script internally hardcodes the use of "cuda:0".
   ...
   # No need to change scale here; initial_lora_path is used as the model initialization

    # These are the current model's LoRA parameters
    lora_params_cache = {
    ...
    dataset_processed = dataset.map(
        lambda example: preprocess_qwen3(
            example,
            tokenizer, 
    ...
    # --- Step 5: Iterative projection calculation loop ---
    projection_scores = []
    ...
    if len(projection_scores) != len(dataset):
        ...
        try:
            ...
            logger.error(f"❌ Error while saving to .pt file: {e}")
