...
def calculate_lora_delta_metrics(
    ...
    if metric not in supported_metrics:
        raise ValueError(f"Unsupported metric: '{metric}'. Supported values: {supported_metrics}")
    results = {}
...
def main(
    ...
    try:
        ...
        def _pick_single_adapter(m):
            ...
            if isinstance(adapter_name_or_list, (list, tuple)):
                if len(adapter_name_or_list) > 0:
                    ...
                    # If it is an empty list, return 'default'
                    return "default"
            
            # If it is already a string (or 'default'), return directly
        ...
        for m in model.modules():
            ...
            # ---- Directly set injection strength (Critical) ----
    ...
    # Current model's LoRA parameters
    lora_params_cache = {
    ...
    # dataset = dataset.select(range(100)) #test
    
    logger.info("Preprocessing dataset...")
    logger.info("Using qwen3_preprocess to handle data")
